...
clear()
inputs_qtd = safe_int_input(f'{choice}Quantas pessoas deseja analisar? > ')
...
for inp in range(1, inputs_qtd + 1):
    ...
    idade = safe_int_input(f"{strTime} Digite o ano de nascimento de {b}{nome}{reset} > ")
    idade = year - idade
    if idade > 120 or idade < 0:
        print(f'{err} A idade não pode ser maior que 120 anos nem menor que 0!')
        quit()

    sexo  = safe_int_input(f"{strTime}Digite o sexo da pessoa {a}[{b}M{a}/{b}F{a}]{reset} >").strip().upper()
    if sexo not in 'MF':
